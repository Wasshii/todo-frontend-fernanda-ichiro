...
// 4. Setup Endpoint GET dan Penangkapan Parameter (Paginasi)
export async function GET(request: NextRequest) {
    ...
    // 5. Eksekusi Pengambilan Data dan Pembungkusan Respons Sukses (GET)
    try {
        ...
        const responsePayload: ApiResponse = {
            success: true,
            message:
                "Koneksi ke DummyJSON API berhasil! Data berhasil diambil.",
            count: result.tasks.length,
            total: result.total,
            data: {
                ...result,
                latency: `${latencyMs}ms`,
                sourceEndpoint: "https://dummyjson.com",
            },
...
// 7. Setup Endpoint POST dan Validasi Payload Dasar
        ...
        return NextResponse.json(
            {
                success: true,
                message:
                    "Berhasil membuat todo baru melalui DummyJSON API (Simulasi)!",
                data: newTodo,
                timestamp: new Date().toISOString(),
            },
            { status: 201 },
